...
<body class="font-[Poppins] pb-[83px]">
    ...
    <nav id="Navbar" class="max-w-[1130px] mx-auto flex justify-between items-center mt-[30px]">
        <div class="logo-container flex gap-[30px] items-center">
            <a href="index.html" class="flex shrink-0">
                <img src="{{asset('assets/images/logos/logopas.png')}}" alt="logo" style="width: 50px; height: auto;" />
            </a>
    ...
    <nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
        ...
        <a href="{{route('front.kategori', $item_kategori->slug)}}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]">
            <div class="w-6 h-6 flex shrink-0">
                <img src="{{Storage::url($kategori->icon)}}" alt="icon" />
            </div>
            <span>{{$item_kategori->nama}}</span>
        </a>
        @endforeach
				...
				<div
					...
					<div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
						<div
							...
							<p class="text-xs leading-[18px] font-bold uppercase">{{$news->kategori->nama}}</p>
						</div>
						<img src="{{Storage::url($news->thumbnail)}}" alt="thumbnail photo"
							class="w-full h-full object-cover" />
					</div>
					<div class="flex flex-col gap-[6px]">
						<h3 class="text-lg leading-[27px] font-bold">{{substr($news->nama, 0, 70)}}{{ strlen($news->nama) > 70 ? '...' : ''}}</h3>
						<p class="text-sm leading-[21px] text-[#A3A6AE]">{{$news->created_at->format('M d, Y')}}</p>
					</div>
				</div>
			</a>
            @empty
            <p>Belum data berita terkait kategori berikut</p>
            @endforelse
	...
	<section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-[70px]">
		<div class="flex flex-col gap-3 shrink-0 w-fit">
			...
			<p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1">
				Kunjungi Iklan Kami <a href="#" class="w-[18px] h-[18px]"><img
